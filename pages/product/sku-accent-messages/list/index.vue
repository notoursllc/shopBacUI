...
import SkuAccentMessageForm from '@/components/product/sku/skuAccentMessage/SkuAccentMessageForm';
...
export default {
    ...
    methods: {
        ...
        async fetchData(paramsObj) {
            try {
                const { data } = await this.$api.productSkuAccentMessages.list(paramsObj);
                this.messages = data;
            }
            catch(e) {
        ...
        async deleteType(data) {
            ...
            try {
                const response = await this.$api.productSkuAccentMessages.delete(data.id);
